...
class EmailNotifier:
    ...
    def send_threat_alert(self, threat_dict):
        ...
        if not self.app.config.get('SEND_EMAIL_NOTIFICATIONS', True):
            log_system('INFO', 'Email notifications are disabled by configuration. Skipping alert delivery.')
            return False
...
"""
            ...
            try:
                # Send the message asynchronously to avoid blocking packet processing.
                thread = threading.Thread(target=self._dispatch_email, args=(msg,), daemon=True)
                thread.start()
                log_system('INFO', f"Alert email dispatched asynchronously to {admin_email} for threat event {threat_dict['id']}")
                return True
